...
export function RiftPage({ save, mutate, setPage }: PageProps) {
  ...
  const canUnlock = character.level >= 30 || character.completedDungeons.includes("black_screen_bay");
  return (
    <div className="page-grid">
      <section className="panel hero-panel">
        ...
        <button className="primary" disabled={!canUnlock || character.status === "archived"} onClick={() => setPage("battle")}>挑战下一层</button>
      </section>
      <section className="panel">
        ...
        <div className="drop-details">
          ...
          <div><span>主要奖励</span><strong>天阶法宝、道纪遗宝、归墟晶片、劫火核心</strong></div>
        </div>
        <button
          disabled={stable <= 0 || character.status === "archived"}
          onClick={() =>
            mutate((draft) => ({
              ...draft,
              idleFarmConfig: {
